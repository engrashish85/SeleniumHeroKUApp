...
import Module9_PageObjectModel.HeroKUAppInfiniteScrollsPage;
import Module9_PageObjectModel.HeroKUAppMainPage;

public class HeroKUAppInfinityScrolls_DataProvider extends Selenium_WorkingWithDriver_TestNG  {
	...
	public void clickHoversLink () throws IOException {
		...
		if (bElementVisible == true) {
			...
			obHeroKUAppMainPage.oInfiniteScroll.click();
			...
			test.log(LogStatus.INFO, "Infinite Scroll link not found");
			
			//Reporting to console
			System.out.println("Infinite Scroll link not found");
	...
	public void scrollInfinite (String sTestCaseID, String sRun, String scrollNumbers) throws IOException {
		...
		HeroKUAppInfiniteScrollsPage obHeroKUAppScrollsPage = PageFactory.initElements(driver, HeroKUAppInfiniteScrollsPage.class);
		
		//Type casting String to Integer
		int scrollNumber = Integer.parseInt(scrollNumbers);
		
		//Setting the slider value as the value given in excel and capturing results in extent reports
		obHeroKUAppScrollsPage.scrollInfinite(scrollNumber);
	...
	public Object[][] returnDataThroughDataProvider () {
		...
		Object[][] rowDataArray = obGetDataFromDataProvider.returnDataFromDataProvider (sProjectPath +
			"\\src\\Module9_TestNG\\DataDriver\\DataDriver_IFrame_TinyMCE_WYSIWYG_Editor.xlsx", "Infinite_Scrolls");
